...
export function RosterFields({ defaults, disabled = false, big = false }: {
  ...
  return (
    <div className={`grid md:grid-cols-3 ${big ? 'gap-6' : 'gap-4'}`}>
      ...
      <p className={`${ui.help} md:col-span-3 ${big ? 'text-base' : ''}`}>Your woman plays both mixed games. Your two men play the men&apos;s doubles together.</p>
    </div>
  );
}
